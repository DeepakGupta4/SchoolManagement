...
export const labSchema = z.object({
  name: z.string().trim().min(2, "Lab name is required"),
  type: z.enum(["Physics", "Chemistry", "Biology", "Computer"]),
  block: z.string().trim().min(2, "Block / location is required"),
  capacity: z.coerce.number<number>().min(0, "Cannot be negative"),
  inCharge: z.string().trim().min(2, "Lab in-charge is required"),
  assistant: z.string().trim().min(2, "Lab assistant is required"),
  ...
  nextPractical: z.string().trim().min(1, "Next practical date is required"),
  nextPracticalClass: z.string().trim().min(2, "Practical details are required"),
